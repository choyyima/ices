...
class Product_Category extends MY_Controller {
    ...
    private $title_icon = 'fa fa-tag';
    
    function __construct(){
        parent::__construct();
        $this->index_url=  get_instance()->config->base_url().'product_category';
    }
    public function add(){
        $this->edit();
    }
    ...
    public function edit($id=""){
        $this->load->helper('master/product_category_engine');

        $action = "Add";
        if(strlen($id)>0) $action = 'Edit';
        if($action != 'Add' && Product_Category_Engine::get($id) == null){
        ...
        if($post != null){
            ...
            if(Product_Category_Engine::save($data) == 1) redirect($this->index_url);
    ...
    {           
        ...
        $row = $app->engine->div_add()->div_set('class','row');            
        $form = $row->form_add()->form_set('title','Product Category List')->form_set('span','12');
        $form->form_group_add()->button_add()->button_set('class','primary')->button_set('value','New Product Category')
                ->button_set('icon','fa fa-plus')->button_set('href',get_instance()->config->base_url().'product_category/add');
        ...
        $tbl->table_ajax_set('id','ajax_table')
                ...
                ->table_ajax_set('lookup_url',$this->index_url.'/ajax_search/product_category')
    ...
    public function view($id = ""){
        ...
        if(Product_Category_Engine::get($id) == null){
            Message::set('error',array("Data doesn't exist"));
            redirect($this->index_url);
        }
        ...
        Product_Category_Engine::detail_render($detail_pane,array("id"=>$id));
