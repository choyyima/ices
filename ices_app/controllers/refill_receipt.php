...
class Refill_Receipt extends MY_Controller {
    ...
    {           
        ...
        $form = $row->form_add()->form_set('title',Lang::get(array('Refill Receipt','List')))->form_set('span','12');
        $form->form_group_add();
        $form->form_group_add()->button_add()->button_set('class','primary')->button_set('value',Lang::get(array('New','Refill Receipt')))
        ...
        $cols = array(
            ...
            array("name"=>"refill_receipt_date","label"=>Lang::get("Date"),"data_type"=>"text"),
            ...
            array("name"=>"refill_receipt_status","label"=>Lang::get("Status"),"data_type"=>"text"),
        );
    ...
    public function view($id="",$method="view"){
        ...
        $this->load->helper($this->path->refill_receipt_engine);
        $this->load->helper($this->path->refill_receipt_data_support);
        ...
        if($cont){
            if(in_array($method,array('view'))){
                if(!Refill_Receipt_Data_Support::refill_receipt_exists($id)){
        ...
        if($cont){
            ...
            if($method === 'view'){
                ...
                Refill_Receipt_Renderer::refill_receipt_allocation_view_render($app,$refill_receipt_allocation_pane,array("id"=>$id),$this->path);
    ...
    public function data_support($method="",$submethod=""){
        ...
        switch($method){
            case 'refill_receipt_get':
                $response =array();
                $db = new DB();
                $refill_receipt_id = Tools::_str($data['data']);
                $q = '
                    select t1.*,
                        t2.code store_code,
                        t2.name store_name,
                ...
                if(count($rs)>0){
                    ...
                    $refill_receipt['deposit_date'] = is_null($refill_receipt['deposit_date'])?
                        null:Tools::_date($refill_receipt['deposit_date'],'F d, Y H:i');
                    ...
                    $refill_receipt['customer_text'] = SI::html_tag('strong',$refill_receipt['customer_code'])
                        .' '.$refill_receipt['customer_name'];
                    $refill_receipt['bos_bank_account_text'] = $refill_receipt['bos_bank_account_code'];
                    ...
                    $refill_receipt['change_amount'] = Tools::thousand_separator($refill_receipt['change_amount']);
                    $refill_receipt['payment_type_text'] = SI::html_tag('strong',$refill_receipt['payment_type_code']);
    ...
    public function refill_receipt_invoiced($id=''){
        
        $this->load->helper($this->path->refill_receipt_engine);
        $post = $this->input->post();
        if($post!= null){
            $param = array('id'=>$id,'method'=>'refill_receipt_invoiced','primary_data_key'=>'refill_receipt','data_post'=>$post);
            SI::data_submit()->submit('refill_receipt_engine',$param);
        }
    ...
    public function refill_receipt_canceled($id=''){
        
        $this->load->helper($this->path->refill_receipt_engine);
        $post = $this->input->post();
        if($post!= null){
            $param = array('id'=>$id,'method'=>'refill_receipt_canceled','primary_data_key'=>'refill_receipt','data_post'=>$post);
            SI::data_submit()->submit('refill_receipt_engine',$param);
        }
