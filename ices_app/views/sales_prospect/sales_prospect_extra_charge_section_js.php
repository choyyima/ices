<script>
    var sales_prospect_extra_charge_section_methods={
        hide_all:function(){
        ...
        reset_all:function(){
            var lparent_pane = sales_prospect_parent_pane;
            var lajax_url = sales_prospect_data_support_url+'mismatch_order_rules_message_get/';
            var ljson_data = {};
            ...
            $(lparent_pane).find('#sales_prospect_mismatch_order_rules')[0].innerHTML = lresponse;
            $(lparent_pane).find('#sales_prospect_extra_charge').val('0.00');
        },
        btn_controller_set:function(){
            ...
            if(lvalid){
                ...
                $(lparent_pane).find('#sales_prospect_btn_next').on('click',function(e){
                    e.preventDefault();
                    sales_prospect_routing.set(lmethod,'payment');
                });
            }
    ...
    sales_prospect_extra_charge_section_bind_events = function(){
        var lparent_pane = sales_prospect_parent_pane;
        var lextra_charge = $(lparent_pane).find('#sales_prospect_extra_charge')[0];
