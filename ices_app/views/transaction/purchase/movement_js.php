<script>
    ...
    var movement_modal_init = function(){
        movement_data.cols = [
            ...
            {name:'product_name',label:'Item',type:'text',visible:true,attr:{style:'text-align:left',col_name:'product_name'}},
        ...
        product = APP_DATA_TRANSFER.ajaxPOST(ajax_search+'movement_po_product',json_data);
        for(var i = 0;i<product.length;i++){
            product[i].row_num = i+1; 
        }
        tbl_component = $("#movement_modal_table_product")[0];
        table = APP_COMPONENT.table.init().component_set(tbl_component).data_set(product).column_set(movement_data.cols);
        table.render();
    }
    ...
    var movement_modal_draw = function(){
        ...
        var qties = $("#movement_modal_table_product").find('[col_name="qty"]');
        $.each(qties,function(){
    ...
    $(function(){
        ...
        $(a).on('click',function(e){
            ...
            product = APP_DATA_TRANSFER.ajaxPOST(ajax_search+'movement_movement_detail',json_data);
            for(var i = 0;i<product.length;i++){
                product[i].row_num = i+1; 
            }            
    ...
    $("#movement_modal_button_save").on('click',function(e){
        ...
        if(method == 'add'){
            var rows = $("#movement_modal_table_product").find('tbody')[0].children;
            $.each(rows,function(){
                row = $(this);
                ...
                json_data.product.push({unit_id:unit_id,product_id:product_id,qty:qty});
            });
        }
