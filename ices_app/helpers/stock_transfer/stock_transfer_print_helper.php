...
class Stock_Transfer_Print {
    ...
    public static function refill_work_order_form_footer_print($p_engine,$rwo_id){
        ...
        if($rwo['refill_work_order_status']!=='initialized'){
            ...
            $p_engine->Ln();
            $p_engine->Cell(0,null,'Telah Terima Deposit total sebesar Rp.'.Tools::thousand_separator(Refill_Work_Order_Data_Support::deposit_total_amount_get($rwo['id'])));
        }
        $p_engine->Ln();
        $p_engine->Ln();
        $p_engine->Ln();
        ...
        if(count($rwo)>0){
            ...
            for($i = 0;$i<count($rwo_product);$i++){
                ...
                if($new_page){
                    ...
                    Refill_Work_Order_Print::refill_work_order_form_header_print($p_engine,$rwo['id']);
                ...
                $p_engine->fpdf->MultiCell($col_width,5,$product_medium,
                    1,($rwo['refill_work_order_status']==='initialized'?'L':'C')
                );
