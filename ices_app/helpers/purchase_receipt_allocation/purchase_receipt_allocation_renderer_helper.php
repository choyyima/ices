<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Purchase_Receipt_Allocation_Renderer {
        
        public static function modal_purchase_receipt_allocation_render($app,$modal){
            $modal->footer_attr_set(array('style'=>'display:none'));
            $modal->header_set(array('title'=>'Purchase Receipt Allocation','icon'=>App_Icon::purchase_receipt()));
            $components = self::purchase_receipt_allocation_components_render($app, $modal,true);
            
        }
        
        public static function purchase_receipt_allocation_render($app,$form,$data,$path,$method){
            get_instance()->load->helper('purchase_receipt_allocation/purchase_receipt_allocation_engine');
            $path = Purchase_Receipt_Allocation_Engine::path_get();
            $id = $data['id'];
            $components = self::purchase_receipt_allocation_components_render($app, $form,false);
            ...
            $js = '
                <script>
                    $("#purchase_receipt_allocation_method").val("'.$method.'");
                    $("#purchase_receipt_allocation_id").val("'.$id.'");
                </script>
            ';             
            $app->js_set($js);
        ...
        public static function purchase_receipt_allocation_components_render($app,$form,$is_modal){
            ...
            $id_prefix = 'purchase_receipt_allocation';
            ...
            $components['id'] = $form->input_add()->input_set('id',$id_prefix.'_id')
             ...
             $form->input_add()->input_set('id',$id_prefix.'_supplier_id')
                    ->input_set('value','')
            ...
            $cust_dep_detail = array(
                ...
                array('name'=>'purchase_receipt_date','label'=>Lang::get('Purchase Receipt Date'))
            );
            
            $form->input_select_detail_add()
                    ->input_select_set('label',Lang::get('Purchase Receipt'))
                    ->input_select_set('icon',App_Icon::purchase_receipt())
                    ->input_select_set('min_length','0')
                    ->input_select_set('id',$id_prefix.'_purchase_receipt')
                    ->input_select_set('min_length','1')
                    ->input_select_set('data_add',array())
                    ->input_select_set('value',array())
                    ->input_select_set('ajax_url',$path->ajax_search.'input_select_purchase_receipt_search/')
                    ->input_select_set('disable_all',true)
                    ->input_select_set('hide_all',true)
                    ->detail_set('rows',$cust_dep_detail)
                    ->detail_set('id',$id_prefix."_purchase_receipt_detail")
                    ->detail_set('ajax_url',$path->data_support.'input_select_purchase_receipt_detail_get/')                    
                ;
            
            $reference_detail = array(
                array('name'=>'amount','label'=>Lang::get('Amount')),
                array('name'=>'outstanding_amount','label'=>Lang::get('Outstanding Amount')),
                array('name'=>'transactional_date','label'=>Lang::get('Transactional Date')),
            );
            ...
            $components['purchase_receipt_allocation_status'] = $form->input_select_add()
                ->input_select_set('label','Status')
                ->input_select_set('icon','fa fa-info')
                ->input_select_set('min_length','0')
                ->input_select_set('id',$id_prefix.'_purchase_receipt_allocation_status')
                ->input_select_set('data_add',array())
                ->input_select_set('value',array())
            ...
            $js = get_instance()->load->view('purchase_receipt_allocation/'.$id_prefix.'_basic_function_js',$param,TRUE);
            $app->js_set($js);
            return $components;
            
        }
        
        public static function purchase_receipt_allocation_status_log_render($app,$form,$data,$path){
            get_instance()->load->helper('purchase_receipt_allocation/purchase_receipt_allocation_engine');
            $path = Purchase_Receipt_Allocation_Engine::path_get();
            ...
            $q = '
                select null row_num
                    ...
                    ,t2.name user_name
                from purchase_receipt_allocation_status_log t1
                    inner join user_login t2 on t1.modid = t2.id
                    inner join purchase_receipt_allocation t3 
                        on t1.purchase_receipt_allocation_id = t3.id
                where t1.purchase_receipt_allocation_id = '.$id.'
                    order by moddate asc
            ';
            $rs = $db->query_array($q);
            for($i = 0;$i<count($rs);$i++){
                $rs[$i]['row_num'] = $i+1;
                $rs[$i]['purchase_receipt_allocation_status_name']  = SI::get_status_attr(                    
                    SI::status_get('Purchase_Receipt_Allocation_Engine',
                        $rs[$i]['purchase_receipt_allocation_status']
                    )['label']
                );
                $rs[$i]['moddate'] = Tools::_date($rs[$i]['moddate'],'F d, Y H:i:s');
                
            }
            $purchase_receipt_allocation_status_log = $rs;
            
            $table = $form->form_group_add()->table_add();
            $table->table_set('id','purchase_receipt_allocation_purchase_receipt_allocation_status_log_table');
            ...
            $table->table_set('columns',array("name"=>"purchase_receipt_allocation_status_name","label"=>"Status",'col_attrib'=>array()));
            $table->table_set('columns',array("name"=>"user_name","label"=>"User",'col_attrib'=>array()));
            $table->table_set('data',$purchase_receipt_allocation_status_log);
        }
