<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class Customer_Refund_Renderer {
        
        public static function modal_customer_refund_render($app,$modal){
            $modal->footer_attr_set(array('style'=>'display:none'));
            $modal->header_set(array('title'=>'Customer Refund','icon'=>App_Icon::money()));
            $components = self::customer_refund_components_render($app, $modal,true);
        }
        
        public static function customer_refund_render($app,$form,$data,$path,$method){
            get_instance()->load->helper('customer_refund/customer_refund_engine');
            $path = Customer_Refund_Engine::path_get();
            $id = $data['id'];
            $components = self::customer_refund_components_render($app, $form,false);
            ...
            $js = '
                <script>
                    $("#customer_refund_method").val("'.$method.'");
                    $("#customer_refund_id").val("'.$id.'");
                </script>
            ';             
            $app->js_set($js);
        ...
        public static function customer_refund_components_render($app,$form,$is_modal){
            ...
            $form->datetimepicker_add()->datetimepicker_set('label',Lang::get('Customer Refund Date'))
                    ->datetimepicker_set('id',$id_prefix.'_customer_refund_date')
            ...
            $components['customer_refund_status'] = $form->input_select_add()
                ->input_select_set('label','Status')
                ->input_select_set('icon','fa fa-info')
                ->input_select_set('min_length','0')
                ->input_select_set('id',$id_prefix.'_customer_refund_status')
                ->input_select_set('data_add',array())
                ->input_select_set('value',array())
            ...
            $js = get_instance()->load->view('customer_refund/'.$id_prefix.'_basic_function_js',$param,TRUE);
            $app->js_set($js);
            return $components;
            
        }
        
        public static function customer_refund_status_log_render($app,$form,$data,$path){
            $config=array(
                'module_name'=>'customer_refund',
