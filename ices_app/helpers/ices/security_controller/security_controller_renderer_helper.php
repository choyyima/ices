...
class Security_Controller_Renderer {
    ...
    public static function modal_security_controller_render($app,$modal){
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title'=>Lang::get('Security Controller'),'icon'=>'fa fa-cogs'));
        $modal->width_set('95%');
        self::security_controller_components_render($app, $modal,true);
        //</editor-fold>
    }

    public static function security_controller_render($app,$form,$data,$path,$method){
        //<editor-fold defaultstate="collapsed">
        $path = Security_Controller_Engine::path_get();
        
        $id_prefix = Security_Controller_Engine::$prefix_id;
        
        $id = $data['id'];
        self::security_controller_components_render($app, $form,false);
    ...
    public static function security_controller_components_render($app,$form,$is_modal){
        $path = Security_Controller_Engine::path_get();            
        $components = array();
        $db = new DB();
        ...
        $components[$id_prefix.'_status'] = $form->input_select_add()
            ...
            ->input_select_set('min_length','0')
            ->input_select_set('id',$id_prefix.'_security_controller_status')
        ...
        if($is_modal){
            $param['detail_tab'] = '#modal_security_controller .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_security_controller';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'].'security_controller/security_controller_basic_function_js',$param,TRUE);
        $app->js_set($js);

    }

    public static function security_controller_status_log_render($app,$form,$data,$path){
        $config=array(
            'module_name'=>'security_controller',
            'module_engine'=>'security_controller_engine',
            'id'=>$data['id']
        );
