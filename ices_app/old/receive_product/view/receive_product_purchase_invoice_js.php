<script>
    var receive_product_purchase_invoice_methods={
        hide_show:function(){
            ...
            switch(lmethod){
                case  'add':                    
                    ...
                    $(lparent_pane).find('#receive_product_purchase_invoice_add_table').closest('div [class*="form-group"]').show();
                    break;
                case 'view':
                    $(lparent_pane).find('#receive_product_store').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#receive_product_reference').closest('div [class*="form-group"]').show();
                    ...
                    $(lparent_pane).find('#receive_product_notes').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#receive_product_receive_product_status').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#receive_product_purchase_invoice_view_table').closest('div [class*="form-group"]').show();
        ...
        enable_disable: function(){
            ...
            switch(lmethod){
                case 'add':
                    ...
                    $(lparent_pane).find('#receive_product_purchase_invoice').select2('enable');
        ...
        purchase_invoice_info_set:function(){
            var lparent_pane = receive_product_parent_pane;
            var lresult = APP_DATA_TRANSFER.ajaxPOST(receive_product_data_support_url+'purchase_invoice/purchase_invoice_detail_get',{data:$(lparent_pane).find('#receive_product_reference').select2('val')});
            ...
            switch($(lparent_pane).find('#receive_product_method').val()){
                case 'add':
                    if(typeof ldata.purchase_invoice_date !=='undefined'){
        ...
        warehouse_from_info_set:function(){
            var lparent_pane = receive_product_parent_pane;
            var lpurchase_invoice_id = $(lparent_pane).find('#receive_product_reference').select2('val');
            var lresult = APP_DATA_TRANSFER.ajaxPOST(receive_product_data_support_url+'purchase_invoice/warehouse_from_detail_get',
            {purchase_invoice_id:lpurchase_invoice_id});
        ...
        init_data:function(){
            ...
            switch(lmethod){
                case 'add':                                        
                    ...
                    ldefault_status = APP_DATA_TRANSFER.ajaxPOST(receive_product_data_support_url+'purchase_invoice/default_status_get');
                    ...
                    var lresult = APP_DATA_TRANSFER.ajaxPOST(receive_product_data_support_url+'purchase_invoice/warehouse_supplier_get/');
                    ...
                    receive_product_purchase_invoice_methods.purchase_invoice_info_set();
                    receive_product_purchase_invoice_methods.warehouse_from_info_set();
                    receive_product_purchase_invoice_methods.load_product_table();
                    break;
                case 'view':          
                    try{
                        var lreceive_product_id = $(lparent_pane).find('#receive_product_id').val();
                        var lajax_url = receive_product_data_support_url+'purchase_invoice/receive_product_get';
                        ...
                        $(lparent_pane).find('#receive_product_purchase_invoice').select2('data',{id:lreceive_product.purchase_invoice_id,text:lreceive_product.purchase_invoice_code}).change();
                        ...
                        lajax_url = receive_product_data_support_url+'purchase_invoice/next_allowed_status';
        ...
        security_set:function(){
            ...
            if(!APP_SECURITY.permission_get('receive_product','purchase_invoice_'+lstatus_label).result){
                lsubmit_show = false;
            }
            ...
            switch(lmethod){
                case 'add':
                        ...
                        if(lpurchase_invoice_id !== ''){
                            var lajax_url = receive_product_data_support_url+'purchase_invoice/purchase_invoice_product_outstanding_get';
                            var json_data = {data: lpurchase_invoice_id};
                            var lresult = APP_DATA_TRANSFER.ajaxPOST(lajax_url, json_data);
                            var lproduct = lresult.response;
                            var ltbody = $(lparent_pane).find('#receive_product_purchase_invoice_add_table').find('tbody')[0];
                            $(ltbody).empty();
                            var lrow_count = 1;
                            ...
                            $.each(lproduct,function(key, val){
                                ...
                                fast_draw.col_add(lrow,{tag:'td',col_name:'invoiced_qty',style:'text-align:right',val:val.invoiced_qty,type:'text'});
                ...
                case 'edit':
                case 'view':
                    var ltbody = $(lparent_pane).find('#receive_product_purchase_invoice_view_table').find('tbody')[0];
                    $(ltbody).empty();
                    lajax_url = receive_product_data_support_url+'purchase_invoice/receive_product_product_get';
                    var lreceive_product_id = $(lparent_pane).find('#receive_product_id').val();
        ...
        submit:function(){
            ...
            switch(lmethod){
                case 'add':
                    ...
                    json_data.purchase_invoice_receive_product = {
                        purchase_invoice_id:$(lparent_pane).find('#receive_product_reference').select2('val')
                    };
                    json_data.receive_product_product=[];
                    var lproduct = $(lparent_pane).find('#receive_product_purchase_invoice_add_table')[0];
                ...
                case 'view':
                    ...
                    var lajax_method = 'purchase_invoice_'+receive_product_methods.status_label_get();
