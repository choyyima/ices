<script>
    ...
    var refill_receipt_init = function(){
        ...
        refill_receipt_reference_extra_param_get = function(){
    ...
    var refill_receipt_methods = {
        hide_all:function(){
            ...
            $(lparent_pane).find('#refill_receipt_print').hide();
            $(lparent_pane).find('#refill_receipt_submit').hide();
        },
        show_hide:function(){
            var lparent_pane = refill_receipt_parent_pane;
            var lmethod = $(lparent_pane).find('#refill_receipt_method').val();
            ...
            switch(lmethod){
                ...
                case 'view':
                    $(lparent_pane).find('#refill_receipt_submit').show();
                    $(lparent_pane).find('#payment_type').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_code').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#refill_receipt_store').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#refill_receipt_customer').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_refill_receipt_date').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#refill_receipt_refill_receipt_status').closest('div [class*="form-group"]').show();
                    $(lparent_pane).find('#refill_receipt_customer_bank_acc').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_bos_bank_name').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_bos_bank_acc').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_amount').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_outstanding_amount').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_change_amount').closest('.form-group').show();
                    $(lparent_pane).find('#refill_receipt_notes').closest('.form-group').show();
                    break;
            }
        },
        ...
        enable_disable:function(){
            ...
            switch(lmethod){
                case 'add':
                    $(lparent_pane).find('#payment_type').select2('enable');
                    $(lparent_pane).find('#refill_receipt_store').select2('enable');
                    $(lparent_pane).find('#refill_receipt_customer').select2('enable');
                    $(lparent_pane).find('#refill_receipt_amount').prop('disabled',false);
                    $(lparent_pane).find('#refill_receipt_notes').prop('disabled',false);
                    break;
                case 'view':
                    $(lparent_pane).find('#refill_receipt_notes').prop('disabled',false);
                    break;
        ...
        reset_all:function(){
            var lparent_pane = refill_receipt_parent_pane;
            var lprefix_id = refill_receipt_component_prefix_id;
            var lmodule_type = $(lparent_pane).find(lprefix_id+'_module_type').select2_data_list();
            $(lparent_pane).find(lprefix_id+'_module_type').select2('data',lmodule_type[0]);
            $(lparent_pane).find(lprefix_id+'_code').val('[AUTO GENERATE]');
            
            APP_FORM.status.default_status_set('refill_receipt',
        ...
        submit:function(){
            ...
            switch(lmethod){
                case 'add':
                    ...
                    json_data.refill_receipt = {
                        ...
                        payment_type_id: $(lparent_pane).find(lprefix_id+'_type').select2('val'),
            ...
            if(result.success ===1){
                ...
                if(refill_receipt_view_url !==''){
                    var url = refill_receipt_view_url+result.trans_id;
                    window.location.href=url;
                }
                else{
                    refill_receipt_after_submit();
                }
            }
        },
    ...
    var refill_receipt_bind_event = function(){
        ...
        refill_receipt_methods.amount.event_set();
        ...
        $(lparent_pane).find(lprefix_id+'_customer').on('change',function(){
            ...
            if($(this).select2('val')!==''){
                ...
                $(lparent_pane).find(lprefix_id+'_type').select2({data:lresponse});
                $(lparent_pane).find(lprefix_id+'_type').select2('data',lresponse[0]);
                $(lparent_pane).find(lprefix_id+'_type').change();
            }
        });
        
        $(lparent_pane).find(lprefix_id+'_type').on('change',function(){
            $(lparent_pane).find(lprefix_id+'_change_amount').prop('disabled',true);
            $(lparent_pane).find('#refill_receipt_customer_bank_acc').prop('disabled',true);
            $(lparent_pane).find('#refill_receipt_bos_bank_name').prop('disabled',true);
            $(lparent_pane).find('#refill_receipt_bos_bank_acc').prop('disabled',true);
            ...
            if($(this).select2('data').code==='CASH'){
                ...
                $(lparent_pane).find('#refill_receipt_customer_bank_acc').prop('disabled',false);
                $(lparent_pane).find('#refill_receipt_bos_bank_name').prop('disabled',false);
                $(lparent_pane).find('#refill_receipt_bos_bank_acc').prop('disabled',false);

            }
        });
        
        
        $(lparent_pane).find('#refill_receipt_submit').off();        
        $(lparent_pane).find('#refill_receipt_submit')
        .on('click',function(e){
            e.preventDefault();
            var btn = $(this);
            ...
            $('#modal_confirmation_submit_btn_submit').on('click',function(){
                refill_receipt_methods.submit();
                $('#modal_confirmation_submit').modal('hide');

            });
            
            $(refill_receipt_window_scroll).scrollTop(0);        
        ...
        var refill_receipt_data_set = function(){
            ...
            switch(lmethod){
                case 'add':
                    ...
                    if(refill_receipt_insert_dummy){
                        
                    }
                    break;
                case 'view':
                    
                    var lrefill_receipt_id = $(lparent_pane).find('#refill_receipt_id').val();
                    ...
                    $(lparent_pane).find('#payment_type').val(lrefill_receipt.payment_type);
                    $(lparent_pane).find('#refill_receipt_store').select2('data',{id:lrefill_receipt.store_id
                        ,text:lrefill_receipt.store_text});
                    $(lparent_pane).find('#refill_receipt_code').val(lrefill_receipt.code);
                    $(lparent_pane).find('#refill_receipt_customer_bank_acc').val(lrefill_receipt.customer_bank_acc);
                    $(lparent_pane).find('#refill_receipt_bos_bank_name').val(lrefill_receipt.bos_bank_name);
                    $(lparent_pane).find('#refill_receipt_bos_bank_acc').val(lrefill_receipt.bos_bank_acc);
                    $(lparent_pane).find('#refill_receipt_amount').val(lrefill_receipt.amount);
                    $(lparent_pane).find('#refill_receipt_outstanding_amount').val(lrefill_receipt.outstanding_amount);
                    $(lparent_pane).find('#refill_receipt_change_amount').val(lrefill_receipt.change_amount);
                    $(lparent_pane).find('#refill_receipt_refill_receipt_date').datetimepicker({value:lrefill_receipt.refill_receipt_date});
                    $(lparent_pane).find('#refill_receipt_refill_receipt_cancellation_reason').val(lrefill_receipt.cancellation_reason);

                    $(lparent_pane).find('#refill_receipt_refill_receipt_status')
                            .select2('data',{id:lrefill_receipt.refill_receipt_status
                                ,text:lrefill_receipt.refill_receipt_status_text}).change();
                    
                    $(lparent_pane).find('#refill_receipt_customer')
                            .select2('data',{id:lrefill_receipt.customer_id
                                ,text:lrefill_receipt.customer_text}).change();
                    
                    $(lparent_pane).find('#refill_receipt_refill_receipt_status')
                            .select2({data:lresponse.refill_receipt_status_list});
                    
                    $(lparent_pane).find('#refill_receipt_notes').val(lrefill_receipt.notes);
