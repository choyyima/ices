...
class Refill_Receipt_Engine {
    ...
    public static $status_list = array(
        ...
        array(
            ...
            ,'msg'=>array(
                'success'=>array(
                    array('val'=>'Add Refill - '),array('val'=>'Receipt'),array('val'=>'success')
                )
            )
        ),
        array(//label name is used for method name
            ...
            ,'msg'=>array(
                'success'=>array(
                    array('val'=>'Update Refill - '),array('val'=>'Receipt'),array('val'=>'success')
                )
            )
        )
        ,array(
            ...
            ,'msg'=>array(
                'success'=>array(
                    array('val'=>'Cancel Refill - '),array('val'=>'Receipt'),array('val'=>'success')
                )
            )
        )
        //</editor-fold>
    ...
    public static function validate($method,$data=array()){  
        ...
        switch($method){
            case 'refill_receipt_add':
                $db = new DB();
                $reference_id = isset($rr_info['reference_id'])?$rr_info['reference_id']:'';
                ...
                $bos_bank_name = isset($refill_receipt['bos_bank_name'])?
                    str_replace(' ','',Tools::_str($refill_receipt['bos_bank_name'])):'0';

                $bos_bank_acc = isset($refill_receipt['bos_bank_acc'])?
                    str_replace(' ','',Tools::_str($refill_receipt['bos_bank_acc'])):'0';

                if(Payment_Type_Data_Support::payment_type_code_get($payment_type_id)!=='CASH'
                    ...
                    if($bos_bank_name ===''){
                        $success = 0;
                        $msg[] = 'Customer Bank Name '.Lang::get('empty',true, false);
                    }
                    if($bos_bank_acc ===''){
                        $success = 0;
        ...
        switch($action){
            case 'refill_receipt_add':
                ...
                $refill_receipt = array(
                    ...
                    'bos_bank_name'=>Tools::empty_to_null($refill_receipt_data['bos_bank_name']),
                    'bos_bank_acc'=>Tools::empty_to_null($refill_receipt_data['bos_bank_acc']),
    ...
    public function refill_receipt_add($db,$final_data,$id){
        ...
        $frefill_receipt_info = $final_data['refill_receipt_info'];
